...
> {
    let uri_str = format!("{}/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/Mobile.json", configuration.base_path, AccountSid=crate::apis::urlencode(params.account_sid), CountryCode=crate::apis::urlencode(params.country_code));
    let mut req_builder =
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListAvailablePhoneNumberMobileResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListAvailablePhoneNumberMobileResponse`")))),
        }
    } else {
        let content = resp.text().await?;

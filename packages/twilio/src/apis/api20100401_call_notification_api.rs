...
pub async fn fetch_call_notification(
    ...
    models::ApiV2010AccountCallCallNotificationInstance,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FetchCallNotificationError> =
...
pub async fn list_call_notification(
    ...
    models::ListCallNotificationResponse,
    Error<ListCallNotificationError>,
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListCallNotificationError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,

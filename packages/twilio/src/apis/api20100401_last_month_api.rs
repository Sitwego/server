...
pub async fn list_usage_record_last_month(
    ...
    models::ListUsageRecordLastMonthResponse,
    Error<ListUsageRecordLastMonthError>,
> {
    let uri_str = format!(
    ...
    if let Some(ref param_value) = params.start_date {
        req_builder =
            req_builder.query(&[("StartDate", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.end_date {
        req_builder =
            req_builder.query(&[("EndDate", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.include_subaccounts {
        req_builder = req_builder
            .query(&[("IncludeSubaccounts", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListUsageRecordLastMonthResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListUsageRecordLastMonthResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListUsageRecordLastMonthError> =

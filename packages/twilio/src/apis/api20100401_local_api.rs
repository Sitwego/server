...
pub async fn list_available_phone_number_local(
    ...
    models::ListAvailablePhoneNumberLocalResponse,
    Error<ListAvailablePhoneNumberLocalError>,
> {
    let uri_str = format!("{}/2010-04-01/Accounts/{AccountSid}/AvailablePhoneNumbers/{CountryCode}/Local.json", configuration.base_path, AccountSid=crate::apis::urlencode(params.account_sid), CountryCode=crate::apis::urlencode(params.country_code));
    let mut req_builder =
        configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = params.area_code {
        req_builder =
            req_builder.query(&[("AreaCode", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.contains {
        req_builder =
            req_builder.query(&[("Contains", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.sms_enabled {
        req_builder =
            req_builder.query(&[("SmsEnabled", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.mms_enabled {
        req_builder =
            req_builder.query(&[("MmsEnabled", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.voice_enabled {
        req_builder =
            req_builder.query(&[("VoiceEnabled", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.exclude_all_address_required {
        req_builder = req_builder
            .query(&[("ExcludeAllAddressRequired", &param_value.to_string())]);
    }
    ...
    if let Some(ref param_value) = params.near_number {
        req_builder =
            req_builder.query(&[("NearNumber", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.near_lat_long {
        req_builder =
            req_builder.query(&[("NearLatLong", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.distance {
        req_builder =
            req_builder.query(&[("Distance", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.in_postal_code {
        req_builder =
            req_builder.query(&[("InPostalCode", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.in_region {
        req_builder =
            req_builder.query(&[("InRegion", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.in_rate_center {
        req_builder =
            req_builder.query(&[("InRateCenter", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.in_lata {
        req_builder =
            req_builder.query(&[("InLata", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.in_locality {
        req_builder =
            req_builder.query(&[("InLocality", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.fax_enabled {
        req_builder =
            req_builder.query(&[("FaxEnabled", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListAvailablePhoneNumberLocalResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListAvailablePhoneNumberLocalResponse`")))),
        }
    } else {
        let content = resp.text().await?;

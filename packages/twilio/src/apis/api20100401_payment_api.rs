...
pub async fn create_payments(
    ...
    multipart_form_params
        .insert("IdempotencyKey", params.idempotency_key.to_string());
    multipart_form_params
        .insert("StatusCallback", params.status_callback.to_string());
    if let Some(param_value) = params.bank_account_type {
        multipart_form_params
            .insert("BankAccountType", param_value.to_string());
    }
    ...
    if let Some(param_value) = params.min_postal_code_length {
        multipart_form_params
            .insert("MinPostalCodeLength", param_value.to_string());
    }
    ...
    if let Some(param_value) = params.payment_connector {
        multipart_form_params
            .insert("PaymentConnector", param_value.to_string());
    }
    ...
    if let Some(param_value) = params.require_matching_inputs {
        multipart_form_params
            .insert("RequireMatchingInputs", param_value.to_string());
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreatePaymentsError> =
...
pub async fn update_payments(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdatePaymentsError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,

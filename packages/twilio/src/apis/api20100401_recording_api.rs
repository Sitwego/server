...
pub async fn delete_recording(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<DeleteRecordingError> =
...
pub async fn fetch_recording(
    ...
    if let Some(ref param_value) = params.include_soft_deleted {
        req_builder = req_builder
            .query(&[("IncludeSoftDeleted", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder =
            req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountRecording`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountRecording`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FetchRecordingError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
...
pub async fn list_recording(
    ...
    if let Some(ref param_value) = params.date_created {
        req_builder =
            req_builder.query(&[("DateCreated", &param_value.to_string())]);
    }
    ...
    if let Some(ref param_value) = params.call_sid {
        req_builder =
            req_builder.query(&[("CallSid", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.conference_sid {
        req_builder =
            req_builder.query(&[("ConferenceSid", &param_value.to_string())]);
    }
    if let Some(ref param_value) = params.include_soft_deleted {
        req_builder = req_builder
            .query(&[("IncludeSoftDeleted", &param_value.to_string())]);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListRecordingResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListRecordingResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListRecordingError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,

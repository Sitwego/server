#[cfg(all(
    feature = "reqwest-multipart",
    feature = "reqwest-middleware-multipart"
))]
use reqwest::multipart::Form;
use reqwest::{Body, Client as VanillaClient, IntoUrl};
pub use reqwest::{
    Client as ReqwClient, Method, Request, Response,
...
pub use reqwest_middleware::{
    ClientBuilder, ClientWithMiddleware as MiddlewareClient,
};
...
impl Client {
    ...
    pub fn request<U: IntoUrl>(
        ...
        match self {
            ...
            #[cfg(feature = "reqwest-middleware")]
            Self::Middleware(c) => {
                RequestBuilder::Middleware(c.request(method, url))
            }
        }
    }
    ...
    pub async fn execute_with_extensions(
        ...
        match self {
            Self::Vanilla(c) => c.execute(req).await.map_err(Into::into),
            Self::Middleware(c) => {
                c.execute_with_extensions(req, ext).await.map_err(Into::into)
            }
        }
    }
}
...
impl RequestBuilder {
    ...
    #[cfg(feature = "reqwest-middleware")]
    pub fn with_extension<T: Clone + Send + Sync + 'static>(
        self,
        extension: T,
    ) -> Self {
        match self {
    ...
    pub fn extensions(&mut self) -> &mut http::Extensions {
        match self {
            Self::Vanilla(_) => {
                panic!("attempted to get extensions of vanilla client")
            }
